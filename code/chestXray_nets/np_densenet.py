...
@slim.add_arg_scope
def _transition_block(inputs, num_filters, avg_pool=True, compression=1.0,
                      scope=None, outputs_collections=None):
  ...
  with tf.variable_scope(scope, 'densenetxxx', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      for i in range(num_dense_blocks - 1):
        ...
        net, num_filters = _transition_block(net, num_filters,avg_pool=False,
      ...
      with tf.variable_scope('final_block', [inputs]):
        net = slim.batch_norm(net)
        postnorm = tf.nn.relu(net)
        net = _global_avg_pool2d(postnorm, scope='global_avg_pool')
      ...
      if num_classes is not None:
        end_points['predictions'] = tf.sigmoid(net, name='predictions')

      cams = slim.conv2d(postnorm,
            num_classes, [1, 1], activation_fn=None,normalizer_fn=None,
            scope='logits',reuse=True)

      cams = tf.image.resize_images(cams,[224,224])
      cams = tf.sigmoid(cams)
      end_points['cams'] = cams
