...
class Multi_hot(ItemHandler):
  """An ItemHandler that returns a parsed Tensor."""

  def __init__(self, tensor_key, shape_keys=None, shape=None, default_value=0, classes=1):
    """Initializes the Tensor handler.
    Tensors are, by default, returned without any reshaping. However, there are
    two mechanisms which allow reshaping to occur at load time. If `shape_keys`
    is provided, both the `Tensor` corresponding to `tensor_key` and
    ...
    if shape_keys:
      keys.extend(shape_keys)
    super(Multi_hot, self).__init__(keys)
  ...
  def tensors_to_item(self, keys_to_tensors):
    ...
    if isinstance(tensor, sparse_tensor.SparseTensor):
      ...
      if shape is not None:
        tensor = array_ops.reshape(tensor, shape)
    tensor = tf.reshape(tensor,[tf.size(tensor),1])
    multi_hot = tf.equal(tensor,tf.cast(classes,tensor.dtype))
    multi_hot = tf.reduce_max(tf.cast(multi_hot,tf.float32),0)
...
class ChestXrayDecoder(data_decoder.DataDecoder):
